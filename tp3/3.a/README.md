...
Razonamiento 2: Al iniciar el sistema el boot manager de UEFI identifica los dispositivos booteables disponibles, por ejemplo: ssd, nvme, pendrive. Estos dispositivos cuentan con una etiqueta de arranque (variable Boot####) dentro de cada una se encuentra el nombre del dispositivo, la ruta de hardware y el archivo .efi. Para identificar si este es un dispositivo booteable se lo determina por un estandard de UEFI donde es necesario que el disco este en formato FAT32 y cuente con la estructura /mnt/EFI/BOOT, en este directorio es donde se encuentra el archivo .efi booteable. Las variables Boot#### son guardadas en la lista de prioridad BootOrder, esta lista puede ser establecida de manera manual o al instalar un sistema operativo (por ejemplo, si entramos al manager de UEFI de un sistema con windows veremos que en primer lugar del orden de arranque por defecto se encuentra el disco donde tenemos instalado el sistema operativo), El UEFI Boot Manager utiliza esta lista para intentar iniciar la imagen .efi de sistema operativo, en caso de fallar el inicio de una de estas imagenes se continuara con el siguiente dispositivo.
